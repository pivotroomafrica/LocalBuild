...
import { fetchRescheduleSlotsAction, rescheduleBookingAction, type RescheduleBookingState } from "@/lib/booking/reschedule";
...
const initialRescheduleState: RescheduleBookingState = {};

/**
 * Reschedule flow (spec sections 1, 8-20) -- reuses the exact same
 * get_bookable_slots() engine as original booking, via
 * fetchRescheduleSlotsAction, which passes this booking's own id as
 * excludeBookingId so its current occupied time never blocks its own
 * candidate slots. Duration/format/expert/price are never inputs here --
 * only a new start time and an optional reason, matching
 * reschedule_booking()'s own fixed parameter list (045).
 */
export function RescheduleModal({
  ...
  // Same "loadKey" derived-loading pattern as BookingPicker (spec section
  // 21's month-by-month loading) -- setState only ever runs inside the
  // .then()/.catch() continuations once the request resolves, never
  // synchronously in the effect body, so there is nothing to desynchronize
  // from a render.
  ...
  const [state, formAction, pending] = useActionState(rescheduleBookingAction, initialRescheduleState);
  ...
  // Local selection state is reset here (on close) rather than via an
  // effect watching `open` -- RescheduleModal itself never unmounts when
  // Modal's own render returns null, so without this the next open would
  // otherwise resume a stale date/slot selection from before.
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Reschedule Session" closeOnEscape={!pending}>
      <div className="flex flex-col gap-4">
        <p className="text-sm text-[var(--color-text-muted)]">
          Choose a new time for this session. The expert, duration, format, and price stay the same.
        </p>
        ...
        {selectedSlot ? (
          <form action={formAction} className="flex flex-col gap-3">
            ...
            <div>
              <label htmlFor="reschedule_reason" className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">
                Reason (optional)
              </label>
              <textarea
                ...
                placeholder="Let the expert know why you're moving this session"
              />
            </div>
